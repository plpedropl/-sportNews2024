...
elseif ($route == 'players') {
	if (isset($_GET['category'])) {
		$categoryId = $_GET['category'];
		Controller::playersByCategories($categoryId);
	} else {
		Controller::error404();
	}
}
...
elseif (isset($_SESSION['role'])) {
	...
	} elseif ($route == 'profileEditMatch') {
		if (isset($id)) {
			ControllerAdmin::matchEditResult($id);
		} else {
			Controller::error404();
		}

	} elseif ($route == 'matchDeleteResult') {
		if (isset($id)) {
			ControllerAdmin::matchDeleteResult($id);
		} else {
			Controller::error404();
		}
