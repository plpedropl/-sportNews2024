...
<?php 
    if(isset($players) && $players) {
        ...
        foreach($players as $player) {
            ...
            echo '      <p class="description" style="font-size: 16px; color: #555;">Возраст: '.$player['age'].'<br><br><span style="color: #555;">'.$player['description'].'</span></p>';
            if ($player['categoryId'] != 3 && $player['categoryId'] != 4 && $player['categoryId'] != 2) {
              echo '<div style="text-align: center;">Клуб: ' . $player['cl1'] . '</div>';
          }
          echo '<div style="margin-top: 15px; text-align: center;">Страна: ' . $player['cl2'] . '</div>';
