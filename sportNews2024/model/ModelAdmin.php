...
class ModelAdmin {
		...
		if(isset($_POST['send'])){
			...
			if($item == null){
				if($description!="" && $newsTitle != ""){
					$sql="INSERT INTO `news` (`title`, `description`, `image`, `date_publish`,`categoryID`)
					VALUES ('$newsTitle','$description','$image', NOW(),'$categoriesSelect')";
					$database = new database();
					$item = $database->executeRun($sql);
					if($item==true){
	...
	public static function editNews($code) {
		...
		if(isset($_POST['send'])){
			...
			$categoriesSelect = $_POST['categoriesSelect'];
			if($description!="" && $newsTitle != ""){
				$sql="UPDATE `news` SET title = '$newsTitle', description = '$description', image = '$image', date_publish='$date_publish', categoryID = '$categoriesSelect' WHERE news.id = ".$newsId; 
				$database = new database();
				$item = $database->executeRun($sql);
				if($item==true){
	...
	public static function addMatch() {
		$result= array(false, "Не удалось добавить матч");
		//читаем данные форм в переменные
		if(isset($_POST['send'])){
			...
			$club2 = $_POST['club2'];
			$score = $_POST['score'];
			$tournamentsSelect = $_POST['tournamentsSelect'];
					$sql="INSERT INTO `tourdetail` (`tournametsId`, `clubid_1`, `clubid_2`, `date`, `location`,`result`)
					VALUES ('$tournamentsSelect','$club1','$club2','$matchDate','$matchVenue','$score')";
					$database = new database();
					$item = $database->executeRun($sql);
					if($item==true){
						$result= array(true, "Успешно добавили игрока");
	...
	//Изменение матча
	public static function editMatch($code) {
		$result = array(false, "Не удалось изменить матч");
		//читаем данные форм в переменные
		if(isset($_POST['send'])) {
			...
			if($MatchDate != "" && $MatchVenue != "") {
				...
				$sql = "UPDATE `tourdetail` SET date = '$MatchDate', location = '$MatchVenue', result = '$score' WHERE id = ".$matchId;
			   
				$database = new database();
				$item = $database->executeRun($sql);
				if($item == true) {
					$result = array(true, "Успешно изменили матч");
				}
			} else {
				$result = array(false, "Не удалось изменить матч");
			}
	...
	//Удаление матча
	public static function deleteMatch($code){
		$result=array(false,"Неудалось удалить матч");
		if(isset($_POST['send'])){
			$matchId = $_POST['matchId'];
			$sql="DELETE FROM `tourdetail` WHERE id = ".$matchId;
			$database = new database();
			$item = $database->executeRun($sql);
			if($item==true){
				$result= array(true, "Матч удален");
			}else{
				$result=array(false, "Неудалось удалить матч");
			} 					
		}
